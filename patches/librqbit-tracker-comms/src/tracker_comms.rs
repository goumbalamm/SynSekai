...
impl TrackerComms {
    ...
    async fn task_single_tracker_monitor_http(&self, mut tracker_url: Url) -> anyhow::Result<()> {
        ...
        loop {
            ...
            // Preserve existing query params (e.g. announce_sig, announce_ts used by
            // private trackers like Ygg) instead of replacing them wholesale.
            let full_query = match tracker_url.query() {
                Some(existing) if !existing.is_empty() => {
                    format!("{}&{}", existing, request_query)
                }
                _ => request_query,
            };
            tracker_url.set_query(Some(&full_query));
